...
En caso de tener MACOs, se puede utilizar macFUSE. Instalar siguiendo las
instrucciones de la [documentación oficial](https://github.com/macfuse/macfuse/wiki/Getting-Started)
...
Existen varios tipos de tests disponibles, pero no se asegura su cobertura 100%.

Para correr test funcionales de las estructuras de datos:
```bash
make test-ht
make test-ft
```

Para correr test funcionales del sistema de archivos:
```bash
bash tests/test_fs.sh <punto de montaje> <path a imagen>
```

Para correr test funcionales del sistema de vigilancia:
```bash
bash tests/test_logging.sh <punto de montaje> <path a imagen>
```


### Crear y montar una imagen de prueba (linux)
