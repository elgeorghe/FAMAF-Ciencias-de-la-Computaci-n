...
inline bool fat_table_is_valid_cluster_number(const fat_table table,
                                              u32 cluster) {
    return cluster >= 1 && cluster < table->num_data_clusters + 2;
}
...
int fat_table_set_next_cluster(fat_table table, u32 cur_cluster,
                                u32 next_cluster) {
    ...
    if (written_bytes <= 0) {
        DEBUG("Error writing the next cluster in FAT table");
        return -1;
    }
