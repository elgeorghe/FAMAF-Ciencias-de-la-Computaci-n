...
#define DEBUG(format, ...)                                             \
    ({                                                                 \
        ...
        fprintf(stderr, "DEBUG %s:%u %s(): ", __FILE__, __LINE__, __func__); \
...
static inline unsigned long bsr(unsigned long n) {
    assert(n != 0); // undefined behavior otherwise

#if defined(__x86_64__) || defined(__i386__)
    // Use bsr via compiler builtin (more portable than inline asm)
    return (8 * sizeof(n) - 1) - __builtin_clzl(n);
#else
    // Use the same logic on non-x86 (e.g., ARM)
    return (8 * sizeof(n) - 1) - __builtin_clzl(n);
#endif
}
